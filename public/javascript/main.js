...
var render = function (data){
	...
	$('#name').text(data.name);
	var sublist = _.template($("#linkList").html(),{data:data.sub});
	var reclist = _.template($("#reclinkList").html(),{data:data.recommend});
	$("#list").html(sublist);
	$('#reclist').html(reclist);
	if(data.data){
		var info = _.template($("#infotemp").html(),{data:data.data});
		$('#info').html(info);
	}else{
		$('#info').html('');
	}
...
var bind = function (){
	var todo = function(e){
		...
		var nodename = pageurl.match(/\/([^\/]+)\/$/);
		var path = window.location.pathname;
		var fromnode = path.match(/\/([^\/]+)\/$/);
		//console.log( type, fromnode[1], nodename );
		if(nodename){
			nodename = nodename[1];
			$.ajax({
				url: "/api/" + type + '/' + fromnode[1] + '/' + nodename,
				type: "GET",
				dataType: "json",
				success: function( data, textStatus, jqXHR ) {
					//console.log(data);
					...
					if(pageurl != window.location){
						window.history.pushState({'path':pageurl, 'a': type, 'y':$(window).scrollTop()},'',pageurl);
	...
	$('a').each(function( key, ele ) {
		...
		if(e.attr('href')){
			if(e.attr('href')[0] == '/'){
				e.unbind("click", todo).bind("click", todo);
			}else{
...
$(window).bind('popstate', function(e) {
	...
	if(e.originalEvent.state){
		if(e.originalEvent.state['a'] == "c") $('.row-offcanvas').addClass('active');
		if(e.originalEvent.state['y']) window.scrollTo(0,e.originalEvent.state['y']);
	}
});
